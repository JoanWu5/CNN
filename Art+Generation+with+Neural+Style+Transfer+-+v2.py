...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(tf.transpose(a_S,[0,3,1,2]),[m,n_C,-1])
    a_G = tf.reshape(tf.transpose(a_G,[0,3,1,2]),[m,n_C,-1])
    ...
    J_style_layer = tf.reduce_sum(tf.squared_difference(GS,GG))/(4*np.power(n_H*n_W,2)*np.power(n_C,2))
